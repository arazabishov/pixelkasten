...
- `group` assigns a `group_id` (uuid4 hex) to each keeper. Members of one logical asset (Live Photo image + video, edited variant) share a `group_id`. Takeout mode buckets by shared sidecar — Live Photo image+video, edited variants, and Motion Photos all share a sidecar in Takeout. Archive mode buckets by `(dirname, name)` where `name` is the literal `basename-without-extension`, with one sibling-gated exception: if the name ends in `-edited` (or a truncated variant) AND an un-edited counterpart exists in the same directory, the un-edited form is used. No `(N)` stripping — surviving `(N)` files have distinct content (dedupe collapsed identical ones) and deserve their own group. For Takeout content, use `--from-takeout` for robust pairing; archive mode cannot recover edits or Live-Photo siblings reliably when filenames alone don't carry the signal (Takeout name truncation, collision-marker ambiguity).
