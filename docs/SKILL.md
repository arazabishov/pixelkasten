...
pixelkasten is a photo-library toolbox. It produces a **working library**: a flat directory of GUID-named media files with per-asset sidecar JSONs under `.pixelkasten/`. The working library is your workspace — your job is to propose album assignments by reading sidecars, asking the user, and writing `proposed_album` via the `propose` tool. A separate `apply --to <dst>` command later turns your proposals into an organized photo library.

## Sidecar schema

Every keeper in the working library has a sidecar at `<library>/.pixelkasten/<filename>.pk.json`:

```json
{
  "dates": ["2024-06-01T14:30:22"],                           // init (reconcile)
  "geo": {"latitude": 52.52, "longitude": 13.40, "altitude": 34.0},  // init (reconcile)
  "album": "Wedding 2019",                                    // init (Takeout source folder; null otherwise)
...
|---|---|---|
| `pixelkasten caption <path>` | Run VLM on one asset, cache caption in sidecar. | `pixelkasten caption library/3f2a1b8c.heic` |
| `pixelkasten similar <path> [--k 20]` | k-NN over `embeddings.npy`; prints JSON. | `pixelkasten similar library/3f2a1b8c.heic --k 5` |
| `pixelkasten cluster [paths…] [--min-cluster-size N]` | HDBSCAN over a scoped slice; prints `{label: [path]}`. | `jq -r … \| pixelkasten cluster` |
...
For reads, the sidecars are the source of truth — use `jq`, `find`, `ls`. **Do not call `exiftool` to inspect dates, geo, or album on these files.** `pixelkasten init` already extracted that data into the sidecar; the file's on-disk EXIF and the sidecar can diverge (e.g., when `--skip-metadata-write` was used), and the sidecar is canonical for the agent's purposes.
...
# Group sidecars by stem (siblings share a stem):
ls .pixelkasten/*.pk.json | sed 's/\.[^./]*\.pk\.json$//' | sort -u
```

To map sidecar paths back to media files in the library, drop the `.pk.json` suffix and replace `.pixelkasten/` with the library root.
...
When you understand what happens to files you don't propose, you'll make better calls about when to propose at all. For each photo, `pixelkasten apply` picks a target in this order:
...
4. No parseable date → `Unsorted/<group_id>.<ext>`
...
- **Don't call `exiftool` for dates, geo, or album.** Read those from the sidecar JSONs under `.pixelkasten/`. The sidecar is canonical; on-disk EXIF can lag (e.g., `--skip-metadata-write` runs) and re-reading it is slower and noisier than `jq`.
- **Don't write to sidecars except via `propose`.** Direct edits skip validation and break group propagation across Live Photo siblings and edited variants.
- **Don't move or rename files in the working library.** It's flat for a reason; `apply --to <dst>` does the organizing later.
- **Don't reorganize the working library's folder structure.** No subdirectories, no rename. The working library is canonical.
- **Don't trigger `apply --to <dst>` on the user's behalf without confirmation.** That's a write-out step that produces user-visible artifacts; let them initiate it.
