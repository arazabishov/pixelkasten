...
def render_link_table(console: Console, result: dict) -> None:
    ...
    table.add_row("Matched with high confidence", str(confidence_counts[3]))
    table.add_row("Matched with medium confidence", str(confidence_counts[2]))
    table.add_row("Matched with satisfactory confidence", str(confidence_counts[1]))
...
def render_reconcile_table(console: Console, manifest: list[dict]) -> None:
    ...
    table.add_row("Errors", str(n_error))
