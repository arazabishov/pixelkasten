...
def _build_library(tmp_path, entries: list[dict]) -> str:
    ...
    Each entry: {"filename": "abc.heic", "dates": [...], "album": ..., "proposed_album": ...}
    """
    lib = tmp_path / "lib"
    pk = lib / ".pixelkasten"
    ...
    for entry in entries:
        ...
        if "proposed_album" in entry:
            sidecar["proposed_album"] = entry["proposed_album"]
        (pk / f"{entry['filename']}.pk.json").write_text(json.dumps(sidecar))
    return str(lib)
...
class TestExportTargets:
    def test_proposed_album_takes_precedence_over_init_album(self, tmp_path):
        ...
        apply_export(lib, dst, ApplyOptions())
    ...
    def test_loose_without_date_goes_to_unsorted(self, tmp_path):
        lib = _build_library(
            tmp_path,
            [
        ...
        # No date -> Unsorted/ with the working-library GUID name preserved
        assert _listing(dst) == ["Unsorted/abc123def.jpg"]
...
class TestGroupBehavior:
    def test_group_members_co_locate_in_same_folder(self, tmp_path):
        ...
        out = _listing(dst)
        # Both members share the timestamp-stem; only the extension differs
        assert out == [
    ...
    def test_propagates_proposed_album_to_siblings_without_it(self, tmp_path):
        ...
        # end up in the proposed album folder.
        lib = _build_library(
            tmp_path,
            [
                {
                ...
                {"filename": "grp.mov", "dates": ["2024-06-01T14:30:22"]},
            ],
        )
        dst = str(tmp_path / "out")
...
class TestForceAndDryRun:
    def test_refuses_when_destination_is_nonempty(self, tmp_path):
        ...
        with pytest.raises(RuntimeError, match="already exists"):
            apply_export(str(lib), str(dst), ApplyOptions())
    ...
    def test_force_overwrites_existing_destination(self, tmp_path):
        ...
        apply_export(str(lib), str(dst), ApplyOptions(force=True))
    ...
    def test_dry_run_writes_nothing(self, tmp_path, capsys):
        ...
        summary = apply_export(str(lib), str(dst), ApplyOptions(dry_run=True))
    ...
    def test_working_library_is_untouched_after_apply(self, tmp_path):
        ...
        apply_export(lib, str(tmp_path / "out"), ApplyOptions())

        # File listing unchanged
        assert _listing(lib) == before
        # Sidecar unchanged — proposed_album persists for re-runs
